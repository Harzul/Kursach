...
def gen_pow_matrix(primpoly: int) -> np.ndarray:
...
def sum(X: np.ndarray, axis=0) -> np.ndarray:  # суммирование по размерности
    return np.bitwise_xor.reduce(X, axis=axis).reshape([(-1), (-1, 1)][axis])  # Xor поскольку в F_2 суммирование == xor


def prod(X: np.ndarray, Y: np.ndarray, pm: np.ndarray) -> np.ndarray:  # Поэлементное произведениематриц
    return np.where(np.logical_and(X != 0, Y != 0), pm[(pm[X - 1, 0] + pm[Y - 1, 0] - 1) % len(pm), 1], 0)


def divide(X: np.ndarray, Y: np.ndarray, pm: np.ndarray) -> np.ndarray:  # Поэлементное деление матриц
    if 0 in Y:
        raise ZeroDivisionError
    return np.where(X != 0, pm[(pm[X - 1, 0] - pm[Y - 1, 0] - 1) % len(pm), 1], 0)


def linsolve(A: np.ndarray, b: np.ndarray, pm: np.ndarray) -> np.ndarray:  # возвращает решение СЛАУ в случае невырожденности A и numpy.nan иначе
...
def minpoly(x: np.ndarray, pm: np.ndarray):
    p = np.array([1])
...
def clean_zeros(p: np.ndarray) -> np.ndarray:
    if p[0] != 0:
        return p
    n = np.flatnonzero(p)
...
def polyval(p: np.ndarray, x: np.ndarray, pm: np.ndarray) -> np.ndarray:  # возвращает значения полинома p для набора элементов x
    p = clean_zeros(p)
...
def polyadd(p1: np.ndarray, p2: np.ndarray) -> np.ndarray:
    if p1.size == p2.size:
        return add(p1, p2)
...
def polyprod(p1: np.ndarray, p2: np.ndarray, pm: np.ndarray) -> np.ndarray:
    p1 = clean_zeros(p1)
    p2 = clean_zeros(p2)
    if p1.size < p2.size:
        p1, p2 = p2, p1
...
def polydiv(p1: np.ndarray, p2: np.ndarray, pm: np.ndarray) -> tuple[np.ndarray, np.any]:
    p1 = clean_zeros(p1)
    p2 = clean_zeros(p2)
    if p1.size < p2.size:
...
def euclid(p1: np.ndarray, p2: np.ndarray, pm: np.ndarray, max_deg=0):
    p1 = clean_zeros(p1)
    p2 = clean_zeros(p2)
